...
// ===== UA Retry Patch =====
// 当订阅返回错误(503/403/空内容等)时，自动使用自定义UA重试
// 利用 Quantumult X 的 retry 机制: $done({retry: {user_agent: "..."}})
;(function() {
  if (!$resource || !content0) return;
  var currentUA = ($resource.user_agent || "").toLowerCase();
  var retryUA = "sing-box";  // 该订阅源需要此UA才能正常返回节点
  // 已经用目标UA重试过了，不再重复
  if (currentUA === retryUA) return;
  // 检测是否为错误响应
  var isBad = !content0 ||
    content0.length < 10 ||
    /503|403|404|502|500|forbidden|service.unavailable|not.found|error|block|invalid/i.test(content0.substring(0, 500));
  if (isBad) {
    $done({retry: {user_agent: retryUA}});
  }
})();
// ===== End UA Retry Patch =====
